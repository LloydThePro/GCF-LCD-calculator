...
void CWindow::CreateBitMapImg(unsigned int width, unsigned int heigth, int x, int y, const char * filename)
{
	m_vBmp.push_back((HBITMAP)LoadImage(0, filename, IMAGE_BITMAP, width, heigth, LR_LOADFROMFILE));
	m_vChildwindow.push_back(CreateWindow("Static", NULL, WS_CHILD | WS_VISIBLE | SS_BITMAP, x, y, width, heigth, this->m_hWinhwn, nullptr, nullptr, nullptr));
	int size = m_vChildwindow.size();
	SendMessage(m_vChildwindow[size - 1], STM_SETIMAGE, IMAGE_BITMAP, (LPARAM)m_vBmp[m_vBmp.size() - 1]);
